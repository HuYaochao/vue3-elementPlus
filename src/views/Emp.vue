        ...
        <el-table :data="state.employeeData" style="width: 100%">
            ...
            <el-table-column label="操作" width="200">
                <template v-slot="scopeSlot">
                    ...
                    <el-button type="danger" :icon="Delete" size="small"  @click="confirmDelete(scopeSlot.row.id)">删除</el-button>
                </template>
            </el-table-column>
        </el-table>
...
const sureAdd = async () => {
    ...
    await addFormRef.value.validate(async (valid) => {
        if (valid) {
            const { data } = await $http.post('/emp/add', addForm)
            if (data.message == "OK") {
                ElNotification({
                    title: 'Success',
                    message: '添加成功',
...
const sureEdit = async () => {
    ...
    await editFormRef.value.validate(async (valid) => {
        if (valid) {
             const { data } = await $http.post(`/emp/update?id=${state.editCurrentId}`, {
                dep: editForm.dep,
...
const getEmployeeData = async () => {
    const { data } = await $http.get('/emp/listByPage', {
        params: {
            pageNum: state.page.pageNum,
            pageSize: state.page.pageSize
        }
    })
    state.employeeData = data.data.rows
